...
// wangmeixia HQ_META_PWRKEY_LONG_PRESS_POWEROFF 20141002
	else if(kpd_enable_lprst&& get_boot_mode() == META_BOOT)
	{
		kpd_print("meta Boot Mode long press reboot selection\n");
#ifdef KPD_PMIC_LPRST_TD_META
		kpd_print("Enable meta mode LPRST\n");
	#ifdef ONEKEY_REBOOT_META_MODE
		mt6325_upmu_set_rg_pwrkey_rst_en(0x01);
		mt6325_upmu_set_rg_homekey_rst_en(0x00);
		mt6325_upmu_set_rg_pwrkey_rst_td(KPD_PMIC_LPRST_TD_META);
	#endif

	#ifdef TWOKEY_REBOOT_META_MODE
		mt6325_upmu_set_rg_pwrkey_rst_en(0x01);
		mt6325_upmu_set_rg_homekey_rst_en(0x01);
		mt6325_upmu_set_rg_pwrkey_rst_td(KPD_PMIC_LPRST_TD_META);
	#endif
#else
		kpd_print("disable meta mode LPRST\n");
		mt6325_upmu_set_rg_pwrkey_rst_en(0x00);
		mt6325_upmu_set_rg_homekey_rst_en(0x00);
#endif
	}
